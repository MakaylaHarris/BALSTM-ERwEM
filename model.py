...
from theano.tensor import alloc
...
class Model(object):
    ...
    def setup_train(self):
        ...
        time_result, _ = scan(fn=step_time, sequences=[time_inputs], non_sequences=time_masks,
        ...
        note_result, _ = scan(fn=step_note, sequences=[note_inputs], non_sequences=pitch_masks,
        ...
        self.update_fun = function(
            inputs=[self.input_mat, self.output_mat],
            outputs=self.cost,
            updates=updates,
            allow_input_downcast=True)

        self.update_thought_fun = function(
            inputs=[self.input_mat, self.output_mat],
    ...
    def setup_predict(self):
        ...
        def step_time(*states):
            ...
            start_note_values = alloc(np.array(0, dtype=np.int8), 2)
            ...
            notes_result, updates = scan(fn=self._predict_step_note, sequences=[time_final],
                                                outputs_info=note_outputs_info)
        ...
        time_result, updates = scan(fn=step_time,
                                           outputs_info=time_outputs_info,
        ...
        self.predict_thought_fun = function(
            inputs=[self.steps_to_simulate, self.conservativity, self.predict_seed],
    ...
    def setup_slow_walk(self):
        ...
        self.walk_input = shared(np.ones((2, 2), dtype='int8'))
        self.walk_time = shared(np.array(0, dtype='int64'))
        self.walk_hiddens = [shared(np.ones((2, 2), dtype=config.floatX)) for layer in
